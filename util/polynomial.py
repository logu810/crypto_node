...
class Polynomial:
    ...
    def parse_polynomial(polynomial):
        # Remove spaces and split by '+' to handle each term
        terms = polynomial.replace(' ', '').split('+')
        
        # Initialize a dictionary to hold coefficients for each degree
        coeffs = {}
        ...
        for term in terms:
            ...
            if match:
                coef = int(match.group(1))  # Extract coefficient
                degree = 0  # Default degree
                
                if match.group(2):  # If 'x' is present
                    if match.group(3):  # If 'x^degree' is present
                        degree = int(match.group(3))
                    else:  # If just 'x' is present, it means degree 1
                        degree = 1
        ...
        # Create a list of coefficients ordered by degree
        coef_list = [coeffs.get(i, 0) for i in range(max_degree + 1)]
        
        return max_degree, coef_list
    
    def parse_secret_polynomial(polynomial):
        # Remove spaces and split by '+' to handle each term
        ...
        for term in terms:
            if term:  # Skip empty terms
                ...
                if match:
                    ...
                    if coef_str == '' or coef_str == '+':  # Implicit +1
                        coef = 1
                    elif coef_str == '-':  # Implicit -1
                        coef = -1
                    else:
                        coef = int(coef_str)  # Extract coefficient
        ...
        # Get the maximum degree (up to which we need coefficients)
        max_degree = max(coeffs.keys(), default=0)
        
        # Create a list of coefficients ordered by degree, ensuring length of at least 8
        coef_list = [coeffs.get(i, 0) for i in range(max(8, max_degree + 1))]
